...
const Navigation = () => {
  ...
  return (
    <div
      style={{ zIndex: 9999 }}
      className={`fixed top-0 left-0 h-full bg-[#000] text-white p-4 transition-all duration-300 ease-in-out ${
        showSidebar ? "w-[75%]" : "w-[4%] hover:w-[15%]"
      } md:w-[15%] md:hover:w-[15%] flex flex-col justify-between`}
      id="navigation-container"
    >
      <div className="flex flex-col justify-center space-y-4">
        <Link to="/" className="flex items-center transition-transform transform hover:translate-x-2">
          <AiOutlineHome className="mr-2 mt-[3rem]" size={26} />
          <span className="hidden md:inline mt-[3rem]">HOME</span>
        </Link>

        <Link to="/shop" className="flex items-center transition-transform transform hover:translate-x-2">
          <AiOutlineShopping className="mr-2 mt-[3rem]" size={26} />
          <span className="hidden md:inline mt-[3rem]">SHOP</span>
        </Link>

        <Link to="/cart" className="flex items-center transition-transform transform hover:translate-x-2">
          <AiOutlineShoppingCart className="mr-2 mt-[3rem]" size={26} />
          <span className="hidden md:inline mt-[3rem]">Cart</span>
          {cartItems.length > 0 && (
            <span className="absolute top-9 text-sm text-white bg-pink-500 rounded-full px-1 py-0">
        ...
        <Link to="/favorite" className="flex items-center transition-transform transform hover:translate-x-2">
          ...
          <span className="hidden md:inline mt-[3rem]">Favorites</span>
          <FavoritesCount />
        </Link>
      </div>
